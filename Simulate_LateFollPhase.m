...
function [res, New, NoNew] = Simulate_LateFollPhase(para,paraPoi,paraOde,Par,tb,te,StartValues,StartTimes,FSHVec,ShowStuff,SaveStuff,DirStuff,MenstCycleDay)
...
%-----------------------------------------------------------------------
%
%values for tracking the follicles are initiated
%index of the follicle we currently working on
FollCounter = 1;                                            
%class to save follicles and their properties
Follicles = FollicleClass(y0Foll,FSHVec(FollCounter),t);  
FollCounter = FollCounter + 1;

%values that needed for the first integrations
TimeCounter=1;
...
%-----------------------------------------------------------------------
%
%variable needed for drug administration 
dd1 = 0;
dosing_events1 = [];
%
%-----------------------------------------------------------------------
...
while (t<te) 
    ...
    M(NumFollicles+34,NumFollicles+34)=0; %alg. eq. for GnRH Antagonist
    
    %event function stops the integration, when ever an ovulation takes
    %place within the intervall tspan
    options = odeset('Mass',M,'events',@(t,y)evaluate_follicle(t,y,paraOde(7),para,count15mm,Par,LH)); %,'RelTol',1e-6,'AbsTol',1e-8
    ...
    if ~isempty(dosing_timeIdx)
        ...
        for i=1:length(dosing_timeIdx)
            ...
            if tspan(1)~=tspan(2)
                [ ti, yi ] = ode15s(@(t,y)testfun_LateFollPhase(t,y,Tovu,Follicles,para,paraOde,Par,dd1), tspan, yInitial, options);
                %Problem, wenn der Integrator nicht weiterkommt und ti leer
                %nur den Startwert enthält -> T leerer Vektor
                T=[T;ti(2:end)];
                Y=[Y;yi(2:end,:)];
                tstart=T(end);
                yInitial=Y(end,:);  
        ...
        [T,Y] = ode15s(@(t,y)testfun_LateFollPhase(t,y,Tovu,Follicles,para,paraOde,Par,dd1),tspan,y0,options);
    end
    ...
    if T(end)==tspan(2)
        ...
        para(1) = 1;
        testyslope = testfun_LateFollPhase(T(end),testyvalues,Tovu,Follicles,para,paraOde,Par,dd1);
    ...
    para(1) = 0;
    res = testfun_LateFollPhase(T(end),LastYValues,Tovu,Follicles,para,paraOde,Par,dd1);
    ...
    if Par(596) == 1 && t > Par(598) || Par(596) == 1 && count >= 3
        break
    end
